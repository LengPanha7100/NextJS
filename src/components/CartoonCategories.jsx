...
const CartoonCategories = ({ dataCartoon }) => {
  ...
  const handleSearch = async (search) => {
    // If no search term or empty string is provided, reset to the original dataCartoon
    if (!search || search.trim() === '') {
      setData(dataCartoon);
      return;
    }else{
      // Perform the search action if a valid search term is given
      const resultSearch = await searchCartoonAction(search);
      console.log("searchComponent", resultSearch);
          // If no results, set data to an empty array
